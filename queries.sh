...
echo "$($PSQL "SELECT name FROM games g FULL JOIN teams t ON g.winner_id = t.team_id WHERE year = 2014 AND round LIKE 'Eigh%' UNION SELECT name FROM games g2 FULL JOIN teams t2 ON g2.opponent_id = t2.team_id WHERE year = 2014 AND round LIKE 'Eigh%' ORDER BY name")"
